...
const getDataFromServer = () => {
  makeServiceCalls("GET", "http://localhost:3000/imageData/1", true)
    .then((responseText) => {
...
getDataFromServer();
/*//fetch api
fetch("http://localhost:3000/imageData/1")
  .then((response) => response.json())
  .then((Data) => {
    document.querySelector("#paragraph").innerHTML = Data.description;
    document.querySelector("#WelcomeHeading").innerHTML = Data.title;
    document.querySelector("#seaImg").src = Data.seaImg;
    document.querySelector("#mountainImg").src = Data.mountainImg;
    document.querySelector("#manImg").src = Data.manImg;
  })
  .catch((error) => {
    console.log("GET Error Status: " + JSON.stringify(error));
  });*/
